// Claude Design 用「現状デザイン」スクショ生成。
// 本番 app.kurofukubo.com をゲストモードで開き、現実的なサンプルデータを localStorage(kk4_guest) に投入して各画面を撮影。
// ゲスト=完全ローカル(localStorage)なので API/CORS の影響なし。出力: shots/redesign/
import { chromium } from 'playwright';
import { mkdirSync } from 'node:fs';

const BASE = process.env.SHOT_BASE || 'https://app.kurofukubo.com/?guest';
const OUT = process.env.SHOT_OUT || 'shots/redesign';
mkdirSync(OUT, { recursive: true });

// ── 現実的なサンプルデータ生成（科目IDは DEFAULT_ACCOUNTS に準拠）──
...
function initScript(theme) {
  return ([ds, th]) => {
    ...
    localStorage.setItem('kk_theme', th);
  };
}

async function dismissModals(p) {
  for (let i = 0; i < 4 && (await p.locator('.mo.open, .modal.open, .onb').count()) > 0; i++) {
...
async function navTo(p, label) {
  // モバイルはサイドバーがハンバーガー内 → 先に開く
  const burger = p.locator('.hamburger');
  if (await burger.count() && await burger.isVisible()) { await burger.click(); await p.waitForTimeout(350); }
  const item = p.locator('.s-item', { hasText: label }).first();
  if (await item.count()) { await item.click(); await p.waitForTimeout(900); }
}

async function shootSet({ browser, theme, viewport, ids, prefix }) {
  const ctx = await browser.newContext({ viewport, deviceScaleFactor: 2 });
  await ctx.addInitScript(initScript(theme), [dataset, theme]);
  ...
  let i = 0;
  for (const id of ids) {
    const [pid, label] = PAGES.find((x) => x[0] === id);
    await navTo(p, label);
    await dismissModals(p);
    const file = `${OUT}/${prefix}${String(++i).padStart(2, '0')}-${pid}.png`;
...
if (!process.env.MOBILE_ONLY) {
  console.log('▶ デスクトップ（ライト, 1440x1200）全13画面');
  await shootSet({ browser: b, theme: 'light', viewport: { width: 1440, height: 1200 }, ids: PAGES.map((x) => x[0]), prefix: 'd-' });

  console.log('▶ デスクトップ（ダーク）主要4画面');
  await shootSet({ browser: b, theme: 'dark', viewport: { width: 1440, height: 1200 }, ids: ['dashboard', 'journal', 'bs', 'credit'], prefix: 'dark-' });
}

console.log('▶ モバイル（ライト, 390x844）主要5画面');
await shootSet({ browser: b, theme: 'light', viewport: { width: 390, height: 844 }, ids: ['dashboard', 'journal', 'ledger', 'credit', 'calendar'], prefix: 'm-' });

await b.close();
console.log('\n✅ 完了 → docs/design-gen/' + OUT);
